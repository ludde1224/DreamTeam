...
public class Sudoku {
	...
	public void put(int row, int col, int value){
		if(row < 0 || row > 9 ||col < 0 || col > 9 || value < 1 || value > 9){
			throw new IllegalArgumentException("Brädet får endast innehålla tal inom intervaller 1-9");
		}
		board[row][col]=value;
	}
 ...
 */
	 ...
	 * @param colNumber
	 * Anger columnnumret till den ruta som ska undersökas
	 * @return
	...
	private int nextRow(int row, int col) { // onï¿½dig parameter med row?
		if (col == 8) {
			return (row + 1);
		} else {
			return row;
